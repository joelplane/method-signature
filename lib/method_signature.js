(function (define) {
  define(function(){
    ...
    var doesntThrow = function(func, args){
      try {
    ...
    var rules = [
      {
        ruleName: 'arrayOfType',
        test: /.*\[\]/, // like "number[]" or "SomeClass[]"
        valueCheck: function(typeString, value, check){
          var baseType = typeString.replace(/\[\]$/, '');
          return doesntThrow(check, ['Array', value, check]) &&
                 (value.length == 0 || doesntThrow(check, [baseType, value[0], check]));
        }
      }, {
        ruleName: 'baseType',
        test: /.*/,
    ...
    var findMatchingRule = function(typeString) {
      ...
      rules.forEach(function(rule){
        ...
        if (!foundFunction && typeTestFunction(typeString)) {
          foundFunction = func;
          foundFunction.ruleName = rule.ruleName;
        }
      });
    ...
    var check = function(expectedType, actualValue, methodName, checkType, index) {
      ...
      var ruleName = checkFunction.ruleName;
      if (!checkFunction(expectedType, actualValue, check)) {
        ...
        var message = 'Expected ' + what + ofMethodName + ' to be a ' + expectedType + ', but got ' + actualValueString + ' (' + valueTypeHint + ') (using rule ' + ruleName + ')';
